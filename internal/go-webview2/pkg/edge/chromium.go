...
// ReleaseFocus moves keyboard focus out of the webview to the previous tab stop
// (typically the parent HWND) and blurs the active DOM element.
func (e *Chromium) ReleaseFocus() {
	if e.controller == nil {
		return
	}
	if err := e.controller.MoveFocus(COREWEBVIEW2_MOVE_FOCUS_REASON_PREVIOUS); err != nil {
		e.errorCallback(err)
	}
	e.Eval(`try{if(document.activeElement&&document.activeElement.blur)document.activeElement.blur();}catch(e){}`)
}
